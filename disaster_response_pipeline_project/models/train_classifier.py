...
def load_data(database_filepath):
    ...
    Returns:
    X: values in the message column
    y: the categories in original df
    ...
    df=df.dropna(subset=df.columns[~df.columns.isin(remove_col)].values,axis=0)
    X = df.message.values
    y = df.iloc[:,4:]
...
def save_model(model, model_filepath):
    ...
    Save the model to a specified path as a Picle file that can be loaded later
...
def main():
    ...
    if len(sys.argv) == 3:  
        database_filepath, model_filepath = sys.argv[1:]  
                #DisasterProjection.db, train_classifier.py
        ...
        print('Please check if the filepath is correct: '\
              'Example: /data/DisasterProjection.db, train_classifier.pkl')
